...
int main(void) {
	...
	while (1) {
		// Servo range(180°) 300 - 2200 us
		// But servo only responds from 350 microseconds
		microseconds = 350 + ((1900 * sum) / 4);
		ticks = 80 * microseconds; // number of ticks = 80 MHz clock speed * time in us
		TIM2->CCR2 = ticks;
		sum = (sum + 1) % 5;
		HAL_Delay(1000);
		/* USER CODE END WHILE */
